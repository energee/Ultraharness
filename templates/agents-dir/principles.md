...
Where a rubric below would flag a guard, it stays silent. "It has one caller", "it has
...
The rule runs both ways, or it becomes a license for defensive bloat. Machinery for a
failure the boundary cannot produce — a retry wrapper around an in-process call, a
version scheme for something never released, a config knob for a load nobody has
observed — is still YAGNI's to flag. Handling a failure that can actually happen:
...
- It backs persisted data — a column, table, migration, or serialized field. Rows
  outlive the code that reads them, and a dropped column is the one fix `git revert`
  cannot undo. Unreferenced is never sufficient here.
